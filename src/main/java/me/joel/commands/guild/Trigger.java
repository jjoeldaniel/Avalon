...
public class Trigger extends ListenerAdapter
{
    ...
    public void onSlashCommandInteraction( @NotNull SlashCommandInteractionEvent event )
    {
        if ( event.getName().equals( "trigger" ) )
        {
            if ( event.getSubcommandName().equals( "new" ) )
            {
                ...
                {
                    String sql =
                            "REPLACE INTO triggers(user_id, trigger_word) values ('" + id + "', '" + triggerWord + "')";
    ...
    {
        if ( !event.isFromGuild() || event.getMember() == null || event.getMember().getUser().isBot() )
        {
            return;
        }
        ...
        for ( String id : triggers.keySet() )
        {
            ...
            if ( trigger.contains( triggers.get( id ) ) )
            {
                ...
                for ( var message : log.getRetrievedHistory() )
                {
                    ...
                    {
                        time_diff = event.getMessage().getTimeCreated().toEpochSecond() - message.getTimeCreated()
                                .toEpochSecond();
                        if ( time_diff <= 15 )
                        {
                            break;
                ...
                // Add trigger message
                String member_name = event.getMessage().getAuthor().getName() + "#" + event.getMessage().getAuthor()
                        .getDiscriminator();
                builder.addField( "",
                        "**[" + TimeFormat.TIME_LONG.now() + "] " + member_name + ":** " + event.getMessage()
                                .getContentRaw(), false );
